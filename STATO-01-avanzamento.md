# STATO-01 — Avanzamento del percorso formativo

**Aggiornato al:** 13/07/2026 · **MANUALE COMPLETO (F1–F6 + M1–M22)** · **Prossima milestone: CORSO 1 IN AULA, 22–23 luglio**

## ✅ Fatto

| Cosa | Versione | Dove |
|---|---|---|
| Architettura del percorso (approvata) | v0.1 | src/ |
| Scheda dati aziendale | **v1.1** | src/ |
| Moduli Fondamenta F1–F6 (md) | F1 v0.2, F2–F6 v0.1 | src/ |
| Deck PowerPoint F1–F6 | F1 v0.3, F2–F6 v0.1 | consegnati in chat (non nel repo) |
| Sito: manuale F1–F6 | v0.3 | index.html |
| Sito: area docente con chiavi richiudibili | v1.0 | aula.html |
| Sito: dispense partecipanti stampabili | v1.0 | dispense.html |
| Sito: fogli riservati (verbale + programma esercizio) | v1.0 | dispense-riservate.html |
| Kit d'aula (programma, note, esercizi, chiavi, quiz) | v1.0 | src/aula/ |
| **Manuale Parte I–II (M1–M6)** | v0.1 | manuale-operativo.html + src/manuale/ |
| **Manuale Parti III–VII (M7–M22)** | v0.1 | ricette.html + src/manuale/ |
| Questionario pre-corso | v1.0 | in dispense.html + src/aula/ |

## 🔜 Prima del 22 luglio (azioni tue)

1. **Girare il questionario al team** (entro ~18/07) e passarmi le risposte → calibro il laboratorio 2.3
2. **Aprire i 6 deck in PowerPoint vero** per il controllo grafiche EMF (unica verifica che non posso fare io)
3. Facoltativo: prova generale della demo 1.1 (copione in aula.html)

## 🔜 Prima del 22 luglio (azioni mie, su tuo via)

- Integrare le risposte del questionario negli esercizi
- Eventuali ritocchi ai deck dopo il tuo controllo in PowerPoint

## ✅ Corso 3

**Corso 3 — AI Software Architect** (solo direzione): 15 moduli teorici, A1–A15.

## 📦 Dopo il corso (backlog)

1. Policy dati (F6 §7): da far adottare alla direzione
3. Processi da documentare (M13): 5 candidati in DATI-01 §8, da confermare
4. Piano dei 90 giorni (M20): parte dopo il corso
5. Revisione semestrale dei [FATTO] (M22): prima scadenza → gennaio 2027

## ❓ Decisioni aperte

- **GitHub Pages** (sito con URL pubblico): richiede prima la sanificazione dei riferimenti a sponsor/persone, oppure piano GitHub a pagamento per Pages su repo privato
- **Token GitHub**: ancora attivo per scelta della direzione — resta la raccomandazione di ruotarlo periodicamente
- Deck pptx nel repo? (ora viaggiano solo in chat)
